...
submitNewPic = () => {
    ...
    const {id, updateProfilePic} = this.props;
    console.log('check', uImage)
    axios.patch(`/api/updatePic/${id}`, {uImage}).then((res)=>{
    ...
    render() {
        ...
        const {username,first_name,last_name,bio,profilePic} = this.props;
        ...
        return (
            <div className="profilep">
                ...
                <div><h2>Username: </h2><div>{username}</div></div>
...
function mapStateToProps(iS){
    const {id,username,first_name,last_name,bio,profilePic}= iS;
    return {
        id,
        username,
        first_name,
